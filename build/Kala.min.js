...
b.position.x=a.center.x;b.position.y=a.center.y;b.width=a.radius?2*a.radius:a.width;b.height=a.radius?2*a.radius:a.height;b.rotation=a.angle};this.updatePolygon=function(a){var b=h[a.bodyID];b.clear();b.lineStyle(1/d,a.lineColor||260348);b.lineColor;b.drawPolygon(a.vertexToPath());for(var c=0;c<a.vertex.length-1;c++)b=a.vertex[c].midpoint(a.vertex[c+1]),g.drawLine(b,b.add(a.faceNormal[c].scale(10/d)));b=a.vertex[0].midpoint(a.vertex[a.vertex.length-1]);g.drawLine(b,b.add(a.faceNormal[a.faceNormal.length-
1].scale(10/d)))};this.clear=function(){h={};this.bodyContainer.removeChildren();this.lineContainer.removeChildren()};this.update=function(a){this.clear();a.hasChanged=!1;a=a.allBodies;for(var b,c=0;c<a.length;c++)(b=h[a[c].bodyID])||(b=this.addShape(a[c])),b.renderUpdate(a[c])}}
...
g=a.allBodies[c];var h=e;if(0!==f.invMass||0!==g.invMass){var k=f,l=g,p=h,m=k.invMass,n=l.invMass,r=p.getDepth()/(m+n)*.8;p=p.getNormal().scale(r);k.move(p.scale(-m));l.move(p.scale(n));k=h.getNormal();l=h.start.scale(g.invMass/(f.invMass+g.invMass));h=h.end.scale(f.invMass/(f.invMass+g.invMass));h=l.add(h);l=h.subtract(f.center);m=h.subtract(g.center);h=f.velocity.add(Vec2(-1*f.angularVelocity*l.y,f.angularVelocity*l.x));n=g.velocity.add(Vec2(-1*g.angularVelocity*m.y,g.angularVelocity*m.x)).subtract(h);
h=n.dot(k);if(!(0<h)){p=Math.min(f.restitution,g.restitution);r=Math.min(f.friction,g.friction);var t=l.cross(k),u=m.cross(k);h=-(1+p)*h/(f.invMass+g.invMass+t*t*f.inertia+u*u*g.inertia);var q=k.scale(h);f.velocity=f.velocity.subtract(q.scale(f.invMass));g.velocity=g.velocity.add(q.scale(g.invMass));f.angularVelocity-=t*h*f.inertia;g.angularVelocity+=u*h*g.inertia;k=n.subtract(k.scale(n.dot(k)));k=k.normalize().scale(-1);l=l.cross(k);m=m.cross(k);n=-(1+p)*n.dot(k)*r;n/=f.invMass+g.invMass+l*l*f.inertia+
m*m*g.inertia;n>h&&(n=h);q=k.scale(n);f.velocity=f.velocity.subtract(q.scale(f.invMass));g.velocity=g.velocity.add(q.scale(g.invMass));f.angularVelocity-=l*n*f.inertia;g.angularVelocity+=m*n*g.inertia}}}},maintainConstraints:function(a){if(!0===a.movement)for(var b=new CollisionInfo,c=0;40>c;c++)for(var d=0;d<a.allConstraints.length;d++)a.allConstraints[d].maintainConstraint(b)},positionalCorrectionFlag:!0}}var Common={};
...
Events.prototype.addCollisionEvent=function(a){this.collisionEvents[a.name]=a};var SupportStruct=function(){this.supportPoint=null;this.supportPointDist=0},bodyIndex=new Indexer;
...
Polygon.prototype.calculateEdges=function(){for(var a=[],b=0;b<this.vertex.length-1;b++)a[b]=this.vertex[b].subtract(this.vertex[b+1]);a[a.length]=this.vertex[this.vertex.length-1].subtract(this.vertex[0]);return a};Polygon.prototype.calculateFaceNormals=function(){for(var a=[],b=0;b<this.edges.length;b++)a[b]=this.edges[b].normalize();return a};Polygon.prototype.vertexToPath=function(){for(var a=[],b=0;b<this.vertex.length;b++)a.push(this.vertex[b].x),a.push(this.vertex[b].y);return a};
Polygon.prototype.findCentroid=function(){for(var a=0,b,c,d,e=Vec2(0,0),f=0;f<this.vertex.length-1;f++)c=this.vertex[f],d=this.vertex[f+1],b=c.x*d.y-d.x*c.y,a+=b,e.x+=(c.x+d.x)*b,e.y+=(c.y+d.y)*b;c=this.vertex[this.vertex.length-1];d=this.vertex[0];b=c.x*d.y-d.x*c.y;e.x+=(c.x+d.x)*b;e.y+=(c.y+d.y)*b;a=(a+b)/2;e.x/=6*a;e.y/=6*a;return e};Polygon.prototype.sortVertices=function(){var a=this.center;this.vertex.sort(function(b,c){return b.angleFromVector(a)-c.angleFromVector(a)})};
var centroidGraphic=new PIXI.Graphics;Polygon.prototype.draw=function(a){centroidGraphic.clear();a.app.stage.addChild(centroidGraphic);centroidGraphic.lineStyle(1/a.scale);centroidGraphic.drawCircle(this.center.x,this.center.y,10/a.scale)};
...
var tmpSupport=new SupportStruct;Rectangle.prototype.findSupportPoint=function(a,b){tmpSupport.supportPointDist=-9999999;tmpSupport.supportPoint=null;for(var c=0;c<this.vertex.length;c++){var d=this.vertex[c].subtract(b);d=d.dot(a);0<d&&d>tmpSupport.supportPointDist&&(tmpSupport.supportPoint=this.vertex[c],tmpSupport.supportPointDist=d)}};
...
function PolygonConfiguration(){this.min;this.max;this.perpMin;this.perpMax;this.minIndex;this.maxIndex;this.normal;this.collisionType=[]}Polygon.prototype.collisionTest=function(a,b){var c=!1;"Polygon"===a.type&&(c=this.collidedPolyPoly(this,a,b));return c};collisionInfoR1=new CollisionInfo;collisionInfoR2=new CollisionInfo;var normalOfCollision;
Polygon.prototype.findInterval=function(a){var b=new PolygonConfiguration;a=a.perp();var c=this.vertex[0].dot(a),d,e=0,f=a;var g=d=c;for(var h=1;h<this.vertex.length;h++)c=this.vertex[h].dot(a),c>g&&(g=c),c<d&&(d=c,e=h,normalOfCollision=f=a);b.min=d;b.max=g;b.minIndex=e;b.normal=f.perp();return b};
Polygon.prototype.collidedPolyPoly=function(a,b,c){a.bodyID>b.bodyID&&(c=b,b=a,a=c);for(c=0;c<a.faceNormal.length;c++){var d=a.faceNormal[c];var e=a.findInterval(d);d=b.findInterval(d);var f=Math.min(e.max-d.min,d.max-e.min);if(d.max<e.min||e.max<d.min)return a.lineColor=null,!1}for(c=0;c<b.faceNormal.length;c++){d=b.faceNormal[c];e=a.findInterval(d);d=b.findInterval(d);var g=Math.min(e.max-d.min,d.max-e.min);if(d.max<e.min||e.max<d.min)return a.lineColor=null,!1}e=f<g?e.normal:d.normal;a.findSupportPoints(e,
b);b.findSupportPoints(e,a);console.log("Polygons are colliding");return!0};Polygon.prototype.collidedPolyRect=function(){};Polygon.prototype.collidedPolyCirc=function(){};Polygon.prototype.findSupportPoints=function(a,b){a=a.perp();for(var c=[],d=99999,e,f,g=0;g<this.vertex.length;g++)e=this.vertex[g].subtract(b.center),e=e.dot(a),e<d&&(d=e,f=c[0]=g);b=f+1>=this.vertex.length?0:f+1;d=0>f-1?this.vertex.length-1:f-1;e=this.vertex[d];f=e.dot(a);e=this.vertex[b];a=e.dot(a);c[1]=f<a?d:b;return c};
