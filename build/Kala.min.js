...
case "Polygon":return this.addPolygon(a)}};this.addSprites=function(a){if(Array.isArray(a))for(var b=0;b<a.length;b++)this.addSprite(a[b]);else this.addSprite(a)};this.removeSprite=function(a){a=this.bodyContainer.getChildAt(a);void 0!=a&&this.bodyContainer.removeChild(a)};this.drawLine=function(a,b){var c=new PIXI.Graphics;c.clear();this.lineContainer.addChild(c);c.lineStyle(1/d,0,1);c.moveTo(a.x,a.y);c.lineTo(b.x,b.y)};this.renderEdge=function(a,b){};this.updateBody=function(a){var b=h[a.bodyID];
b.position.x=a.center.x;b.position.y=a.center.y;b.width=a.radius?2*a.radius:a.width;b.height=a.radius?2*a.radius:a.height;b.rotation=a.angle};this.updatePolygon=function(a){var b=h[a.bodyID];b.clear();b.lineStyle(1/d,a.lineColor||260348);b.lineColor;b.drawPolygon(a.vertexToPath())};this.clear=function(){h={};this.bodyContainer.removeChildren();this.lineContainer.removeChildren()};this.update=function(a){a.hasChanged&&(this.clear(),a.hasChanged=!1);a=a.allBodies;for(var b,c=0;c<a.length;c++)a[c].draw(this),
(b=h[a[c].bodyID])||(b=this.addShape(a[c])),b.renderUpdate(a[c])}}
function Physics(){return{collision:function(a){var b,c,d,e=new CollisionInfo;for(d=0;40>d;d++)for(b=0;b<a.allBodies.length;b++)for(c=b+1;c<a.allBodies.length;c++)if(a.allBodies[b].boundTest(a.allBodies[c])&&a.allBodies[b].collisionTest(a.allBodies[c],e)){0>e.getNormal().dot(a.allBodies[c].center.subtract(a.allBodies[b].center))&&e.changeDir();if(!0===a.allBodies[b].isSensor||!0===a.allBodies[c].isSensor)return e.bodyA=a.allBodies[b],e.bodyAIndex=b,e.bodyB=a.allBodies[c],e.bodyBIndex=c,e;var f=a.allBodies[b],
g=a.allBodies[c],h=e;if(0!==f.invMass||0!==g.invMass){var k=h.getNormal();var l=h.start.scale(g.invMass/(f.invMass+g.invMass));h=h.end.scale(f.invMass/(f.invMass+g.invMass));l=l.add(h);h=l.subtract(f.center);var n=l.subtract(g.center);l=f.velocity.add(Vec2(-1*f.angularVelocity*h.y,f.angularVelocity*h.x));var m=g.velocity.add(Vec2(-1*g.angularVelocity*n.y,g.angularVelocity*n.x)).subtract(l);l=m.dot(k);if(!(0<l)){var p=Math.min(f.restitution,g.restitution),u=Math.min(f.friction,g.friction),r=h.cross(k),
t=n.cross(k);l=-(1+p)*l/(f.invMass+g.invMass+r*r*f.inertia+t*t*g.inertia);var q=k.scale(l);f.velocity=f.velocity.subtract(q.scale(f.invMass));g.velocity=g.velocity.add(q.scale(g.invMass));f.angularVelocity-=r*l*f.inertia;g.angularVelocity+=t*l*g.inertia;k=m.subtract(k.scale(m.dot(k)));k=k.normalize().scale(-1);h=h.cross(k);n=n.cross(k);m=-(1+p)*m.dot(k)*u;m/=f.invMass+g.invMass+h*h*f.inertia+n*n*g.inertia;m>l&&(m=l);q=k.scale(m);f.velocity=f.velocity.subtract(q.scale(f.invMass));g.velocity=g.velocity.add(q.scale(g.invMass));
f.angularVelocity-=h*m*f.inertia;g.angularVelocity+=n*m*g.inertia}}}},maintainConstraints:function(a){if(!0===a.movement)for(var b=new CollisionInfo,c=0;40>c;c++)for(var d=0;d<a.allConstraints.length;d++)a.allConstraints[d].maintainConstraint(b)},positionalCorrectionFlag:!1}}var Common={};(function(){Common.globalID=0;Common.extend=function(a,b){b=Object.create(b.prototype);b.constructor=a;a.prototype=b;return a};Common.incrementIndexer=function(){return Common.globalID+=1}})();
function Indexer(){this.bodyIndex=-1}Indexer.prototype.incrementIndex=function(){return this.bodyIndex+=1};function Events(){this.customEvents={};this.collisionEvents={}}Events.prototype.addCustomEvent=function(a){this.customEvents[a.name]=a};Events.prototype.addCollisionEvent=function(a){this.collisionEvents[a.name]=a};var SupportStruct=function(){this.supportPoint=null;this.supportPointDist=0},bodyIndex=new Indexer;
...
CollisionInfo.prototype.setInfo=function(a,b,c){this.depth=a;this.normal=b;this.start=c;this.end=c.add(b.scale(a))};CollisionInfo.prototype.changeDir=function(){this.normal=this.normal.scale(-1);var a=this.start;this.start=this.end;this.end=a};var Polygon=function(a,b,c,d,e){this.vertex=a.slice();a=this.findCentroid();Body.call(this,a.x,a.y,b,c,d,e);this.sortVertices();this.type="Polygon";this.edges=this.calculateEdges();this.faceNormal=this.calculateFaceNormals();this.AABB=this.calculateAABB();this.updateInertia()};
Common.extend(Polygon,Body);Polygon.prototype.move=function(a){for(var b=0;b<this.vertex.length;b++)this.vertex[b]=this.vertex[b].add(a);this.edges=this.calculateEdges();this.faceNormal=this.calculateFaceNormals();this.center=this.center.add(a);return this};Polygon.prototype.rotate=function(a){this.angle+=a;for(var b=0;b<this.vertex.length;b++)this.vertex[b]=this.vertex[b].rotate(this.center,a);this.edges=this.calculateEdges();this.faceNormal=this.calculateFaceNormals();return this};
Polygon.prototype.calculateEdges=function(){for(var a=[],b=0;b<this.vertex.length-1;b++)a[b]=this.vertex[b].subtract(this.vertex[b+1]);a[a.length]=this.vertex[this.vertex.length-1].subtract(this.vertex[0]);return a};Polygon.prototype.calculateFaceNormals=function(){for(var a=[],b=0;b<this.edges.length;b++)a[b]=this.edges[b].normalize();return a};
Polygon.prototype.calculateAABB=function(){for(var a=999999,b=-999999,c=999999,d=-999999,e=0;e<this.vertex.length;e++){var f=this.vertex[e];f.x>b&&(b=f.x);f.x<a&&(a=f.x);f.y>d&&(d=f.y);f.y<c&&(c=f.y)}return{minBounds:Vec2(a,c),maxBounds:Vec2(b,d)}};Polygon.prototype.calculateBoundingCircle=function(){for(var a=-99999,b,c=0;c<this.vertex.length-1;c++)b=this.center.subtract(this.vertex[c]).lenth(),b<a&&(a=b)};
Polygon.prototype.vertexToPath=function(){for(var a=[],b=0;b<this.vertex.length;b++)a.push(this.vertex[b].x),a.push(this.vertex[b].y);return a};Polygon.prototype.findCentroid=function(){for(var a=0,b,c,d,e=Vec2(0,0),f=0;f<this.vertex.length-1;f++)c=this.vertex[f],d=this.vertex[f+1],b=c.x*d.y-d.x*c.y,a+=b,e.x+=(c.x+d.x)*b,e.y+=(c.y+d.y)*b;c=this.vertex[this.vertex.length-1];d=this.vertex[0];b=c.x*d.y-d.x*c.y;e.x+=(c.x+d.x)*b;e.y+=(c.y+d.y)*b;a=(a+b)/2;e.x/=6*a;e.y/=6*a;return e};
Polygon.prototype.sortVertices=function(){var a=this.center;this.vertex.sort(function(b,c){return b.angleFromVector(a)-c.angleFromVector(a)})};Polygon.prototype.draw=function(a){var b=new PIXI.Graphics;a.app.stage.addChild(b);b.lineStyle(.05,0);b.drawPolygon([this.AABB.minBounds.x,this.AABB.minBounds.y,this.AABB.maxBounds.x,this.AABB.maxBounds.y])};
...
var RegularPoly=function(a,b,c,d,e,f,g,h){var k=[],l=Vec2(a,b);c=Vec2(l.x-c,l.y);var n=360/d*(Math.PI/180);console.log(n);for(var m=0;m<d;m++)k.push(c),c=c.rotate(l,n);Polygon.call(this,k,e,f,g,h);console.log(this.center);console.log("x: "+a+" y: "+b)};Common.extend(RegularPoly,Polygon);Rectangle.prototype.collisionTest=function(a,b){return"Circle"===a.type?this.collidedRectCirc(a,b):this.collidedRectRect(this,a,b)};var tmpSupport=new SupportStruct;
Rectangle.prototype.findSupportPoint=function(a,b){tmpSupport.supportPointDist=-9999999;tmpSupport.supportPoint=null;for(var c=0;c<this.vertex.length;c++){var d=this.vertex[c].subtract(b);d=d.dot(a);0<d&&d>tmpSupport.supportPointDist&&(tmpSupport.supportPoint=this.vertex[c],tmpSupport.supportPointDist=d)}};
...
Polygon.prototype.findInterval=function(a){a=a.perp();var b;var c=b=this.vertex[0].dot(a);for(var d=1;d<this.vertex.length;d++){var e=this.vertex[d].dot(a);e>c&&(c=e);e<b&&(b=e)}return{min:b,max:c}};
Polygon.prototype.polygonOverlaps=function(a){for(var b,c,d,e,f={minNormal:null,minOverlap:999999},g=0;g<this.faceNormal.length;g++){d=this.faceNormal[g];b=this.findInterval(d);c=a.findInterval(d);e=Math.min(b.max-c.min,c.max-b.min);if(c.max<b.min||b.max<c.min)return this.lineColor=null,!1;e<f.minOverlap&&(f.minOverlap=e,f.minNormal=d.perp())}for(g=0;g<a.faceNormal.length;g++){d=a.faceNormal[g];b=this.findInterval(d);c=a.findInterval(d);e=Math.min(b.max-c.min,c.max-b.min);if(c.max<b.min||b.max<c.min)return this.lineColor=
null,!1;e<f.minOverlap&&(f.minOverlap=e,f.minNormal=d.perp())}return f};
Polygon.prototype.collidedPolyPoly=function(a,b,c){if(a.bodyID>b.bodyID){var d=b;b=a;a=d}d=a.polygonOverlaps(b);var e=b.polygonOverlaps(a);if(!d||!e)return!1;d=d.minOverlap<e.minOverlap?d:e;d.minNormal.perp().scale(d.minOverlap);this.dotTest=d.minNormal.dot(b.center.subtract(a.center));0>this.dotTest&&(d.minNormal=d.minNormal.scale(-1),Vec2(0,0));a.findSupportPoints(d.minNormal.scale(-1),b);a=b.findSupportPoints(d.minNormal,a);d.minNormal.scale(-1).dot(a[0]);d.minNormal.perp().scale(d.minOverlap);
c.setInfo(d.minOverlap,d.minNormal,a[0]);return!0};Polygon.prototype.collidedPolyRect=function(){};Polygon.prototype.collidedPolyCirc=function(){};
Polygon.prototype.findSupportPoints=function(a){for(var b=[],c=99999,d,e,f,g=0;g<this.vertex.length;g++)e=this.vertex[g],d=e.dot(a),d<c&&(c=d,b[0]=e,f=g);d=f+1>=this.vertex.length?0:f+1;g=0>f-1?this.vertex.length-1:f-1;e=this.vertex[g];f=e.dot(a);e=this.vertex[d];a=e.dot(a);b[1]=f===c?this.vertex[g]:a===c?this.vertex[d]:f<a?this.vertex[g]:this.vertex[d];return b};
