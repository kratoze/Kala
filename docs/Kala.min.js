...
function PixiRender(a,b,c,d){var f;function e(){f=PIXI.Sprite.from("imgs/circle.png").texture;switch(c){case "stone":l=g.loader.resources.sheet.textures["elementMetal011.png"];break;case "space":l=g.loader.resources.ship.texture;f=g.loader.resources.asteroid1.texture;break;case "pool":l=g.loader.resources.wood.texture}}var g=this;this.app=new PIXI.Application({height:a,width:b,backgroundColor:14137255,antialias:!0});var h=[14272499,15976432,4145535,14417882];this.scale=d;document.body.appendChild(this.app.view);
this.app.stage.scale.x=this.app.stage.scale.y=d;var k={};this.bodyContainer=new PIXI.Container;this.lineContainer=new PIXI.Container;this.loader=new PIXI.Loader;c=c||"stone";var l=f=void 0;switch(c){case "space":this.loader.add("ship","imgs/AsteroidGame/enemyBlue3.png").add("asteroid1","imgs/AsteroidGame/meteorBrown_big1.png").add("explosion","imgs/AsteroidGame/ExplosionAnimation.json").load(e);break;case "stone":this.loader.add("sheet","imgs/spritesheet_metal.json").load(e);break;case "pool":this.loader.add("wood",
"imgs/PoolGameImgs/woodTexture.jpg").add("whiteball","imgs/PoolGameImgs/whiteball.png").add("poolballs","imgs/PoolGameImgs/redball.png").add("pot","imgs/PoolGameImgs/pot.png").load(e)}this.app.stage.addChild(this.bodyContainer);this.app.stage.addChild(this.lineContainer);this.addRectangle=function(a){k[a.bodyID]=new PIXI.Sprite(a.render.texture||l);k[a.bodyID].anchor.x=k[a.bodyID].anchor.y=.5;this.bodyContainer.addChild(k[a.bodyID]);k[a.bodyID].renderUpdate=this.updateBody;return k[a.bodyID]};this.addCircle=
function(a){k[a.bodyID]=new PIXI.Sprite(a.render.texture||f);k[a.bodyID].anchor.x=k[a.bodyID].anchor.y=.5;k[a.bodyID].height=k[a.bodyID].width=2*a.radius;this.bodyContainer.addChild(k[a.bodyID]);k[a.bodyID].renderUpdate=this.updateBody;return k[a.bodyID]};this.addPolygon=function(a){k[a.bodyID]||(k[a.bodyID]=new PIXI.Graphics,k[a.bodyID].color=h[Math.floor(Math.random()*h.length)],this.bodyContainer.addChild(k[a.bodyID]));k[a.bodyID].renderUpdate=this.updatePolygon;return k[a.bodyID]};this.addShape=
function(a){switch(a.type){case "Rectangle":return this.addRectangle(a);case "Circle":return this.addCircle(a);case "Polygon":return this.addPolygon(a)}};this.addSprites=function(a){if(Array.isArray(a))for(var b=0;b<a.length;b++)this.addSprite(a[b]);else this.addSprite(a)};this.removeSprite=function(a){a=this.bodyContainer.getChildAt(a);void 0!=a&&this.bodyContainer.removeChild(a)};var m=new PIXI.Graphics;this.drawLine=function(a,b){m.clear();this.lineContainer.addChild(m);m.lineStyle(1/d,0,1);m.moveTo(a.x,
a.y);m.lineTo(b.x,b.y)};this.renderEdge=function(a,b){};this.updateBody=function(a){var b=k[a.bodyID];b.position.x=a.center.x;b.position.y=a.center.y;b.width=a.radius?2*a.radius:a.width;b.height=a.radius?2*a.radius:a.height;b.rotation=a.angle};this.updatePolygon=function(a){var b=k[a.bodyID];b.clear();b.lineStyle(0/d,a.lineColor||260348);b.lineColor;b.beginFill(k[a.bodyID].color,1);b.drawPolygon(a.vertexToPath());b.endFill()};this.clear=function(){k={};this.bodyContainer.removeChildren();this.lineContainer.removeChildren()};
...
a.allBodies[b],g=a.allBodies[c];var h=f;if(0!==e.invMass||0!==g.invMass){var k=e,l=g,m=h,n=k.invMass,p=l.invMass,r=m.getDepth()/(n+p)*.8;m=m.getNormal().scale(r);k.move(m.scale(-n));l.move(m.scale(p));k=h.getNormal();l=h.start.scale(g.invMass/(e.invMass+g.invMass));h=h.end.scale(e.invMass/(e.invMass+g.invMass));h=l.add(h);l=h.subtract(e.center);n=h.subtract(g.center);h=e.velocity.add(Vec2(-1*e.angularVelocity*l.y,e.angularVelocity*l.x));p=g.velocity.add(Vec2(-1*g.angularVelocity*n.y,g.angularVelocity*
n.x)).subtract(h);h=p.dot(k);if(!(0<h)){m=Math.min(e.restitution,g.restitution);r=Math.min(e.friction,g.friction);var t=l.cross(k),u=n.cross(k);h=-(1+m)*h/(e.invMass+g.invMass+t*t*e.inertia+u*u*g.inertia);var q=k.scale(h);e.velocity=e.velocity.subtract(q.scale(e.invMass));g.velocity=g.velocity.add(q.scale(g.invMass));e.angularVelocity-=t*h*e.inertia;g.angularVelocity+=u*h*g.inertia;k=p.subtract(k.scale(p.dot(k)));k=k.normalize().scale(-1);l=l.cross(k);n=n.cross(k);p=-(1+m)*p.dot(k)*r;p/=e.invMass+
g.invMass+l*l*e.inertia+n*n*g.inertia;p>h&&(p=h);q=k.scale(p);e.velocity=e.velocity.subtract(q.scale(e.invMass));g.velocity=g.velocity.add(q.scale(g.invMass));e.angularVelocity-=l*p*e.inertia;g.angularVelocity+=n*p*g.inertia}}}},maintainConstraints:function(a){if(!0===a.movement)for(var b=new CollisionInfo,c=0;15>c;c++)for(var d=0;d<a.allConstraints.length;d++)a.allConstraints[d].maintainConstraint(b)},positionalCorrectionFlag:!0}}var Common={};
...
var RegularPoly=function(a,b,c,d,f,e,g,h){var k=[];a=Vec2(a,b);c=Vec2(a.x-c,a.y);b=360/d*(Math.PI/180);for(var l=0;l<d;l++)k.push(c),c=c.rotate(a,b);Polygon.call(this,k,f,e,g,h)};Common.extend(RegularPoly,Polygon);Rectangle.prototype.collisionTest=function(a,b){var c=!1;"Circle"===a.type?c=this.collidedRectCirc(a,b):"Rectangle"===a.type?c=this.collidedRectRect(this,a,b):"Polygon"===a.type&&(c=a.collidedPolyRect(a,this,b));return c};var tmpSupport=new SupportStruct;
...
Polygon.prototype.collidedPolyCirc=function(a,b){var c=null,d=999999,f=this.findClosestVertex(a).subtract(a.center).normalize();var e=this.findInterval(f);var g=a.findInterval(f);var h=Math.min(e.max-g.min,g.max-e.min);if(g.max+a.radius<e.min||e.max<g.min-a.radius)return!1;h<d&&(d=h+a.radius,c=f.perp());for(var k=0;k<this.faceNormal.length;k++){var l=this.faceNormal[k];e=this.findInterval(l);g=a.findInterval(l);h=Math.min(e.max-g.min,g.max-e.min);if(g.max+a.radius<e.min||e.max<g.min-a.radius)return!1;
h<d&&(d=h+a.radius,c=l.perp())}e=f.scale(a.radius);b.setInfo(d,c.scale(-1),a.center.add(e));return!0};
